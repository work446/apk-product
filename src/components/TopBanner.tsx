    ...
    <div className="fixed top-0 left-0 w-full z-[60] bg-primary text-primary-foreground py-2 px-4 text-xs md:text-sm font-medium h-10 md:h-9 flex flex-col justify-center">
      <div className="container mx-auto flex flex-row justify-between items-center gap-2 md:gap-0">
        ...
        <div className="flex items-center gap-2 tracking-wide uppercase">
          {activeBanner.centerIcon &&
            typeof activeBanner.centerIcon === 'object' &&
            (activeBanner.centerIcon.imagekit?.url || activeBanner.centerIcon.url) && (
              <Image
                src={
                  (activeBanner.centerIcon.imagekit?.url || activeBanner.centerIcon.url) as string
                }
                alt={activeBanner.centerIcon.alt || 'Center Icon'}
                width={activeBanner.centerIcon.width || 24}
                height={activeBanner.centerIcon.height || 24}
                className="w-5 h-5 object-contain"
              />
            )}
